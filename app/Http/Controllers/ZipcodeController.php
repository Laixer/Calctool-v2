...
class ZipcodeController extends Controller {
    ...
    public function getExternalAddress(Request $request)
    {
        ...
        if (Cache::has($request->get('zipcode') . $request->get('number'))) {
            $address = Cache::get($request->get('zipcode') . $request->get('number'));
            $address['cache'] = true;
        ...
        if (count($data->_embedded->addresses) == 1) {
            ...
            Cache::put($request->get('zipcode') . $request->get('number'), $address, 43800);
